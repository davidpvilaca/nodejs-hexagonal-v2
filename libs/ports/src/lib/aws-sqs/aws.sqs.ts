...
import { throwCustomError, EClassError } from '@nodejs-hexagonal-v2/utils'
 ...
 */
export const receiveMessage = <T>(sqs: SQS, queueUrl: string, maxNumberOfMessages = 1) => async <A = T>(visibilityTimeout = 20, waitTimeSeconds = 10): Promise<SQSMessageResult<readonly (A | null)[], SQS.ReceiveMessageResult>> => {
 ...
 */
export const deleteMessage = (sqs: SQS, queueUrl: string) => async (receiptHandle: string): Promise<SQSMessageResult<null, unknown>> => {
